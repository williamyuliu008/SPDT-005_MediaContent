...
### 2.6 挖掘模式（Fallback：当标签召回为空时）

若标签召回结果为空，触发挖掘模式——调用 LLM 对原始史料进行实时扫描：

```yaml
mining_mode:
  trigger: "标签召回候选节点数 < 3"
  actions:
    - "LLM 扫描史料文本，识别潜在 dramatic_irony"
    - "识别 moral_dilemma（两难困境）"
    - "识别 structural_contradiction（结构性矛盾）"
    - "利用 relations 字段进行图谱推理（precedes/contradicts/compares_with）"
    - "动态生成标签（临时标注，不入库）"
  output:
    - "动态标签：{#dynamic_dramatic_irony, #dynamic_moral_dilemma, ...}"
    - "候选节点列表（带动态标签）"
    - "标注来源：mining_mode（区别于预标注节点）"
  confidence_threshold: 0.6  # 置信度 <0.6 的动态标签不采用
```
...
**降级策略（Fallback）**：
- 若 top2 得分差 < 0.05，或 Layer 3 后无唯一胜出 → 默认采用安全模板
  - 开篇/中间章节 → TPL-01（小事撬动大局）
  - 结尾章节 → TPL-09（长时段缓慢变化）
- 标记 Warning，不阻断流程
```
...
> **密度量化算法**：密度不由字数/信息熵决定，由**模板类型**直接决定。校验引擎读取模板ID即得密度级别，无需额外计算。

```yaml
breathing_rules:
  density_classification:
    high: [TPL-03, TPL-06, TPL-09]    # 深度分析、制度悲剧、长时段
    medium: [TPL-01, TPL-04, TPL-05]  # 叙事+分析、文明碰撞、技术改变
    low: [TPL-07, TPL-10]             # 感性/多视角、被遗忘的多数

  constraints:
    - "高密度单元后必须跟中或低密度单元"
    - "连续三个中密度后必须插入低密度"
    - "结尾必须是中低密度（留给读者思考空间）"
    - "禁止 tension_level 连续下降（除刻意制造'虚假安全感'）"
```
...
| 等级 | 说明 | 参与 Agent | 宽容度 |
|------|------|-----------|--------|
| 轻度 | 草稿自检，仅规则对抗 | 规则引擎（关键词/张力曲线等） | 宽松 |
| 中度 | 内部打磨 | 规则 + 史观审计员 + 读者代言人（LLM对抗） | 标准 |
| 重度 | 出版前终审 | 全部5个Agent | 严格 |

#### 4.3.1 评审宽容度参数（review_strictness）

在 Series Profile 中配置，控制评审严格程度：
...
review_config:
  ...
  confidence_threshold: 0.6  # 置信度低于此值的 Warning 不阻断
```

#### 4.3.2 置信度加权机制

每个 issue 包含 `confidence` 字段（0.0-1.0）：

| severity | confidence | 处理规则 |
|----------|-----------|---------|
| error | ≥ 0.5 | 阻断 |
| warning | ≥ 0.6 | Warning 记录，不阻断 |
| warning | < 0.6 | 低置信度参考，仅记录，不计入阻断判断 |
...
### 5.3.1 上下文感知填充（Context-Aware Filling）

各 Slot 不独立，前序 Slot 的输出内容影响后续 Slot 的指代连贯性：

```yaml
slot_chain:
  dependency: ordered  # 按章节结构顺序填槽
  prefix_context: true  # 后续 Slot 接收前序已填充文本
  coherence_check:
    enabled: true
    checks:
      - "代词指代一致性（前序人名/地名不丢失）"
...
### 5.3.3 Token 计数器与成本监控

每次 LLM 调用记录 Token 消耗，超预算触发告警。
...
### A. 反思智能体（Reflection Agent）——评审失败后的建设性翻译层

| 属性 | 内容 |
|------|------|
| 定位 | 将抽象评审意见翻译为可执行修改指令，不创作不攻击 |
| 触发 | review_fail → 主编 reroute → 反思智能体 |
| 输入 | 评审报告（dimension/severity/location/issue/suggestion/confidence） |
| 输出 | `edit_instruction_json`（具体 slot_id + 修改参数） |
| 关键能力 | 置信度解读（<0.6 → 仅参考不要求修改）、位置映射（sec_02 → slot_id）、指令翻译 |
...
```json
{
  "edit_instruction": {
    ...
    "reason": "reader_advocate warning, confidence=0.68"
  }
}
```

### B. 合规扫描智能体（Compliance Scan Agent）——配置漂移检测

| 属性 | 内容 |
|------|------|
| 定位 | 文风包/史观包升级后，自动扫描存量内容是否符合最新配置 |
| 触发模式 | 手动 / 定时 / 配置变更时自动 |
| 扫描目标 | forbidden_words / 史观关键词 / 文风包句式偏好 |
| 输出 | 受影响章节列表 + 违规详情 + 修复建议 |

### C. Error Code 标准化

| Error Code | 含义 | 恢复策略 |
|-----------|------|---------|
| `ERR_NO_TEMPLATE_FOUND` | 三层漏斗无候选模板 | 安全模板降级（TPL-01） |
| `ERR_REVIEW_LOOP_MAX` | 修订循环达到上限 | 人工干预通道 |
| `ERR_LLM_SAFETY_TRIGGER` | LLM 输出触发安全词 | 打断重试，最多2次 |
| `ERR_FORBIDDEN_WORD_LEAK` | 输出后校验发现 Forbidden Word | 替换重试 |
| `ERR_TOKEN_BUDGET_EXCEED` | Token 超预算 | 降级小模型或 fallback |
| `ERR_STYLE_JS_DIVERGENCE` | 风格偏离 JS >0.15 | 强制 Top-1 词汇 |
| `ERR_REVIEW_CONFLICT` | 多评审 Agent 意见冲突 | 主编仲裁 |
| `ERR_SNAPSHOT_INCONSISTENT` | 快照读取不符 | 重新加载重试 |

### D. 幂等性要求

断点续跑依赖每个环节的**幂等输出**：
- 选材/编排阶段：完全幂等（规则引擎，相同输入→相同输出）
- 生成阶段：Slot 输出缓存，不重复调用 LLM（允许词汇差异）
