...
TONE_RULES = {
    "assertive": {
        ...
        "banned": ["毫无疑问", "绝对", "必然如此", "蠢", "恶", "汉奸", "卖国",
                    "欢迎在评论区", "欢迎评论区", "欢迎留言", "请在评论区", "请留言",
                    "欢迎分享", "请转发", "欢迎讨论", "请在下面留言",
                    "让我们一起", "一起推动这场讨论", "转发给"],
        "structure": "七段式（钩子→论点→支撑→对立→反驳→结论）",
    },
    "scholarly": {
        "allowed": ["研究表明", "学术共识认为", "从X理论视角看"],
        "banned": ["毫无疑问", "绝对", "必然如此", "笔者认为",
                    "欢迎在评论区", "欢迎评论区", "欢迎留言", "请在评论区",
                    "欢迎分享", "请转发"],
...
class RenderOpinion:
    ...
    def _render_llm(
        ...
        prompt = (
            ...
            f"结构要求：六段式（钩子→论点→支撑→对立→反驳→结论），结论要有力收束\n"
            f"【重要】禁止包含任何CTA段落（禁止邀请读者评论/转发/讨论/留言，禁止\"欢迎在评论区\"、\"请转发\"、\"让我们一起\"等表达）\n\n"
            f"段落规范（严格遵守）：\n"
            f"- 每段不超过5行，约80-120字，2-3句\n"
            f"- 用句号(。)断句，禁止长句连成一团\n"
            f"- 禁止写超长段落（超200字视为严重质量缺陷）\n\n"
            f"文章大纲：\n{sections_str}\n\n"
            f"输出要求：\n"
            f"1. 标题使用 H1，章节使用 H2\n"
            f"2. 引用来源格式：【A】来源名 或 【B】来源名\n"
            f"3. 结尾加分割线 --- 和平台免责条款，但免责条款前不能有CTA邀请\n"
    ...
    def _check_tone(self, markdown: str, brand_voice: str) -> dict:
        ...
        for pattern in cta_patterns:
            if re.search(pattern, markdown):
                violations.append(f"发现CTA/情感动员：{pattern}")
