#!/usr/bin/env python3
"""
AI 瞭望台 · 网站构建（Phase 3 日报版）
────────────────────────────────────
读取 channels/YYYY-MM-DD.md → 渲染为 HTML → 生成搜索索引

用法: python build_site.py [--date YYYY-MM-DD]
"""
...
def build_index(date_str):
    ...
    companies_seen = set()
    tags_seen = set()
    channel_name = _section_to_channel(current_section)
    ...
    for sig in signals:
        item = {
            ...
            "sentiment": "positive",  # default from CI Engine
...
def _section_to_channel(section):
...
def main():
    ...
    # 日期导航
    yesterday = (datetime.strptime(date_str, '%Y-%m-%d') - __import__('datetime').timedelta(days=1)).strftime('%Y-%m-%d')
    tomorrow = (datetime.strptime(date_str, '%Y-%m-%d') + __import__('datetime').timedelta(days=1)).strftime('%Y-%m-%d')
